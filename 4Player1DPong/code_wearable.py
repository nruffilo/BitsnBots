# This code is for the Trinket m0 version, with custom PCB
...
pixels = neopixel.NeoPixel(board.D0,(4*num_pixels), brightness=0.1, auto_write=False)
pixels.show()
print(pixels)
...
while True:
    ...
    for index, button in enumerate(buttons):
        #print("Values: " + str(button.value), end=" ")
        if button.value < 1000:
            start_loop = start_loop + 1
            if start_loop > 50:
                start_game = True
        if buttons_previous_state[index] > 9000 and button.value < 1000:
            ...
            #we are not in the middle, so animate a strip
                ...
                if ball_location[1] == (num_pixels-2) and ball_direction == -1:
                    if pressed_correct[ball_location[0]]:
                        #cpx.play_file("Coin.wav")
                        fillStrip(ball_location[0],GREEN)
